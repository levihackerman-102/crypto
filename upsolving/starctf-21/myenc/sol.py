...
'''
send m = 0 twice to get ct1 = iv + q**x1 + q**x2 + ... and ct2 = iv + q**y1 + q**y2 + ... so q = gcd(ct1 - ct2, n)
now we have q and by extension p so we can find iv using that fact that flag starts with "*CTF{"
brute force the remaining part of the flag bit by bit
'''